...
class _CardStackState extends State<CardStack> {
  ...
  Future<void> _onSlideOutComplete(SlideDirection direction) async {
    ...
    switch (direction) {
      case SlideDirection.left:
        currentMatch.nope();
       await ActionsToUsers(action: "dislike",user1: Global.userData.userId,user2: currentMatch.profile.id);
        break;
      case SlideDirection.right:
        currentMatch.like();
        await ActionsToUsers(action: "like",user1: Global.userData.userId,user2: currentMatch.profile.id);
        CreateChat(false);
        break;
      case SlideDirection.up:
        currentMatch.superLike();
        await ActionsToUsers(action: "superlike",user1: Global.userData.userId,user2: currentMatch.profile.id);
       CreateChat(true);
        break;
    }
